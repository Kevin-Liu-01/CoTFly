...
assert.equal(deployment.git?.deploymentEnabled, true, 'Git pushes must run the Vercel validation build');
assert.match(deployment.buildCommand, /npm run build/, 'deployment must run the localization prebuild gate');

console.log('gt-enforcement.selftest: config, secrets boundary, builds, and Vercel gate verified');
