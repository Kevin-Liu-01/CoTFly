...
let flyRuntime: import('./fly/runtime.ts').FlyRuntime | null = null;
const settings = createSettingsAccess({
  ...
  isBattleActive: () => !flyRuntime?.autonomous && battlePhase.canOpenBattleSettings(),
...
const battleFrame = createBattleFrameRuntime({
  ...
  getRigMode: () => rig.mode,
  stepSimulation: () => {
    if (flyRuntime?.paused) return;
    flyRuntime?.beforeStep();
    simStep(game, bus, currentWorld(), rig, worldRuntime.collider);
    flyRuntime?.afterStep();
  },
...
const frameLoop = createFrameLoopScheduler({
  ...(flyFrameDocument ? {
    documentState: flyFrameDocument, inputTarget: flyFrameDocument.inputTarget,
  } : {}),
  tick: mainFrame.tick,
  isBootComplete: () => bootComplete,
...
if (flyEmbedRequested) {
  ...
  flyRuntime = installFlyRuntime({
    ...
    action: (action) => {
      ...
      else if(action==='firstAid') input.tapVirtual('consumable2');
      else if(action==='extinguish') input.tapVirtual('consumable3');
      else if(action==='missile') {
        const slot=game.player?.spec.gun.shells.findIndex(shell=>shell.guided);
        if(slot!==undefined&&slot>=0) bus.emit('ui:shellSelect',{slot});
      } else input.tapVirtual('shell1');
    },
    onConsumableUsed: (listener) => { bus.on('ui:consumableUsed',(event)=>{
      if(!event||typeof event!=='object')return;
      const value=event as {slot?:unknown;readyAt?:unknown};
      if(Number.isInteger(value.slot)&&Number.isFinite(value.readyAt))listener(Number(value.slot),Number(value.readyAt));
    }); },
